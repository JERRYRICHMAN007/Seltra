...
>(
  (
    ...
    // Shrink to icon rail as the viewport gets smaller so nav stays available.
    React.useEffect(() => {
      if (openProp !== undefined) return;
      const openState = !isCompact;
      if (setOpenProp) {
        setOpenProp(openState);
      } else {
        _setOpen(openState);
      ...
      document.cookie = `${SIDEBAR_COOKIE_NAME}=${openState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`;
    }, [isCompact, openProp, setOpenProp]);
...
>(
  (
    ...
    // Icon mode keeps a persistent icon rail on all screen sizes (no offcanvas sheet).
    ...
    return (
      <button
        ...
        className={cn(
          ...
          // Don't steal clicks from icon buttons when the rail is collapsed to icons.
    ...
    <Comp
      ...
      data-sidebar="group-label"
        className={cn(
          ...
          // Hide fully when collapsed — opacity-only still intercepts clicks on icons below.
